...
namespace InazumaSearchLib.Core.Crawl.Work
{
    ...
    public class DocumentFileUpdate : WorkBase
    {
        ...
        )
        {
            ...
            // データの登録処理
            ApplicationBase.ExtractFileSuccess extResSuccess;

            // 拡張子に応じてテキストを抽出する
            try
            {
                var extRes = _app.ExtractFile(FilePath, textExtNames, pluginExtNames);
                if (extRes is ApplicationBase.ExtractFileSuccess)
                {
                    extResSuccess = (ApplicationBase.ExtractFileSuccess)extRes;
                    Logger.Debug($"Extract OK - {FilePath} (title: {extResSuccess.Title}, body length: {extResSuccess.Body.Length})");
                }
                else
                {
                    // テキスト抽出失敗
                    var extResFailed = (ApplicationBase.ExtractFileFailed)extRes;
            ...
            // Groongaへデータを登録
            var appVer = Environment.GetLibraryVersion();
