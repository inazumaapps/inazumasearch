...
namespace InazumaSearchLib.Plugin
{
    public class API : IPluginAPI
    {
        public virtual IDictionary<string, string> TextExtNameToLabelMap { get; set; }
        public virtual IDictionary<string, TextExtractHandler> TextExtractHandlers { get; set; }

        public API()
        {
            TextExtNameToLabelMap = new Dictionary<string, string>();
            TextExtractHandlers = new Dictionary<string, TextExtractHandler>();
        }
        ...
        public virtual void RegisterTextExtractor(string formatTitle, string[] extensions, TextExtractHandler handler)
        {
            foreach (var ext in extensions)
            {
                TextExtNameToLabelMap[ext] = formatTitle;
                TextExtractHandlers[ext] = handler;
            }
        }
    }
}
